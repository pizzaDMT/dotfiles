...
# Matrix Rain Effect for PowerShell 7.5 - With Catppuccin Frappe color pattern
# Anti-flicker version with double buffering
# Modified with peach lead character followed by dimmed blue, lavender, mauve repeating pattern
...
# Using slightly dimmed Catppuccin Frappe colors with bright pale pink lead
$PINK_LEAD = "$([char]27)[38;2;244;184;228m"     # Frappe Pink (brightened) for lead character
$SAPPHIRE = "$([char]27)[38;2;116;155;194m"      # Frappe Sapphire (dimmed)
$LAVENDER = "$([char]27)[38;2;186;187;241m"      # Frappe Lavender (dimmed)
$MAUVE = "$([char]27)[38;2;198;160;246m"         # Frappe Mauve (dimmed)
$TRAIL_GREY = "$([char]27)[38;2;131;139;167m"    # Frappe Overlay0 for trailing characters
$RESET = "$([char]27)[0m"
...
function Show-MatrixRain {
    try {
        ...
        $Host.UI.RawUI.WindowTitle = "catppuccin-frappe-rain"
        [Console]::Clear()
            ...
            foreach ($drop in $drops) {
                ...
                for ($j = 0; $j -lt $totalLength; $j++) {
                    ...
                    if ($posY -ge 0 -and $posY -lt $height -and $posX -ge 0 -and $posX -lt $width) {
                        # Select color based on position in drop - peach lead, then sapphire, lavender, mauve pattern
                        $color = $TRAIL_GREY  # Default for trailing characters
                        $char = $drop.Chars[$j % $drop.Chars.Count]
                        
                        if ($j -eq 0) { 
                            # Lead character is bright pale pink
                            $color = $PINK_LEAD 
                        }
                        elseif ($j -lt $drop.Length) {
                            # Apply repeating Catppuccin Frappe pattern based on position
                            switch (($j - 1) % 3) {
                                0 { $color = $SAPPHIRE }  # First position after lead is sapphire
                                1 { $color = $LAVENDER }  # Second position after lead is lavender
                                2 { $color = $MAUVE }     # Third position after lead is mauve
                            }
                        }
                        else {
                            # Trailing characters use overlay0 grey
                            $color = $TRAIL_GREY
                        }
